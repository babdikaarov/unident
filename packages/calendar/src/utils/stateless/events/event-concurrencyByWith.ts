...
export const handleEventConcurrencyByWith = (
  ...
  for (let i = currentIndex; i < sortedEvents.length; i++) {
    ...
    if (
      ...
      return handleEventConcurrencyByWith(
        sortedEvents,
        concurrentEventsCache,
        i + 1
      )
...
export const groupEventsByWithOptimized = (
  events: CalendarEventInternal[]
): { id: number; name: string; events: CalendarEventInternal[] }[] => {
  const map = new Map<
    number,
    { id: number; name: string; events: CalendarEventInternal[] }
  >()
  for (const event of events) {
    const foreign = event._getForeignProperties()
    // console.log(foreign)
    const { id, name } = foreign.with
    if (!map.has(id)) {
