...
class CurrentTimePluginImpl implements CurrentTimePlugin {
  ...
  onRender($app: CalendarAppSingleton): void {
    ...
    // This could be your target date value

    this.observer = new MutationObserver((mutationList) => {
      for (const mutation of mutationList) {
        if (mutation.type === 'childList') {
          // Find matching element
          const match = calendarWrapper.querySelector<HTMLElement>(
            `.sx__week-grid-staff[data-time-grid-date-wrapper="${this.todayDateString}"]`
          )

          if (match) {
            console.log('found you', match)
            // You could call setIndicator or other logic here
            this.setIndicator()
          }
        }
      }
